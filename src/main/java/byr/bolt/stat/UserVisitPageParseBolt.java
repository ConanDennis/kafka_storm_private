...
public class UserVisitPageParseBolt extends BaseBasicBolt {
    ...
    private static final long serialVersionUID = -6832795820912321558L;
    ...
    public void cleanup() {
        ...
        for (String userName : userList){
            ...
            if(max != null && max.doubleValue() > 0){
                List<Double> collect = values.stream().map(v -> max == 0 ? 0 : (double) (Math.round(v / max) / 100.0)).collect(Collectors.toList());
